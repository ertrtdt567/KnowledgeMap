...
DEFAULT_INPUT = "work/oop_kg_demo/data/sample_questions.json"
...
def normalize_question(raw: dict[str, Any], index: int) -> tuple[dict[str, Any] | None, list[str]]:
    ...
    if not gold_points:
        errors.append(f"{question_id}: gold_knowledge_points 至少需要 1 个标准知识点")
    ...
    question = {
        ...
        "source": {
            "kind": "manual_sample",
            "file": "data/sample_questions.json",
            "index": index,
        },
    }
...
def build_report(
    ...
    return {
        "schema_version": "oop_kg_question_processing_v1",
