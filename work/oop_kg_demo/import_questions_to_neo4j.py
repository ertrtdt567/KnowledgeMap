...
DEFAULT_LINKS = "work/oop_kg_demo/output/question_mapping/question_knowledge_links.json"
DEFAULT_OUTPUT_DIR = "work/oop_kg_demo/output/neo4j_import"
...
class QuestionCypherBuilder:
    ...
    def build_demo_queries_text(self) -> str:
        queries = [
            (
                "1. 查看习题-知识点整体网络",
                "MATCH p=(q:Question)-[:ASSESSES]->(k:KnowledgeNode)\nRETURN p\nLIMIT 100;",
            ),
            (
                "2. 查看某个知识点对应的习题",
                "MATCH p=(q:Question)-[r:ASSESSES]->(k:KnowledgeNode {name: '多态'})\nRETURN p\nLIMIT 50;",
            ),
            (
                "3. 查看一道题考察什么",
            ...
            (
                "4. 查询代码阅读题及其知识点",
                "MATCH p=(q:Question)-[:HAS_TYPE]->(:QuestionType {name: '代码阅读题'})\nMATCH p2=(q)-[:ASSESSES]->(:KnowledgeNode)\nRETURN p, p2\nLIMIT 80;",
            ),
            (
                "5. 查询中等难度题",
                "MATCH p=(q:Question)-[:HAS_DIFFICULTY]->(:Difficulty {name: '中等'})\nRETURN p\nLIMIT 80;",
            ),
            (
                "6. 按知识点统计习题数量",
                "MATCH (:Question)-[r:ASSESSES]->(k:KnowledgeNode)\nRETURN k.name AS knowledge, count(r) AS question_count\nORDER BY question_count DESC;",
            ),
            (
                "7. 按题型统计习题数量",
                "MATCH (q:Question)-[:HAS_TYPE]->(t:QuestionType)\nRETURN t.name AS type, count(q) AS question_count\nORDER BY question_count DESC;",
            ),
            (
                "8. 查看主考知识点关系",
                "MATCH p=(q:Question)-[r:ASSESSES {role: 'primary'}]->(k:KnowledgeNode)\nRETURN p\nLIMIT 80;",
            ),
        ]
        lines = [
            "// 编程领域知识图谱：习题 Demo 查询",
    ...
    def _knowledge_link_statement(self, question_id: str, link: dict[str, Any], mapping_method: str) -> str:
        ...
        return (
            ...
            f"MERGE (q)-[r:ASSESSES {{id: {cypher_literal(rel_id)}}}]->(k)\n"
...
def build_report(
    ...
    return {
        "schema_version": "oop_kg_question_neo4j_import_v1",
...
def main(argv: list[str]) -> int:
    ...
    print(f"ASSESSES 关系数量：{report['assesses_relation_count']}")
