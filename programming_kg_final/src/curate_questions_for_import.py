...
def is_approved(question: dict[str, Any], min_confidence: float) -> bool:
    ...
    # 来源答案必须同时满足：题干证据、答案证据、题号/题序唯一性与位置顺序均已校验。
    # 不能仅凭答案文本非空或“provided”标签自动入库。
    if source in {"provided", "source_provided"}:
        return (
            str(question.get("answer_status", "")) in {"source_verified", "verified"}
            and pairing.get("status") == "verified"
            and bool(str(question.get("answer", "")).strip())
        )
    return (
        source == "llm_generated"
        and str(question.get("answer_status", "")) in {"human_verified", "compiler_verified"}
        and float(question.get("answer_confidence", 0.0) or 0.0) >= min_confidence
        and bool(str(question.get("answer", "")).strip())
        and str(question.get("verification_method", "")) not in {"", "same_model_review"}
    )
...
def main(argv: list[str]) -> int:
    ...
    write_json(
        review_path,
        {
            "schema_version": "programming_kg_question_curation_v2",
