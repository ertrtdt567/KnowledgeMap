...
class Neo4jExecutor:
    ...
    def execute(self, statements: list[str]) -> None:
        """执行通用 Cypher 语句列表，保留给生成脚本等小规模任务使用。"""
        try:
    ...
    def execute_graph(
        ...
        """按标签和关系类型分批写入正式图谱，避免逐条提交造成大量网络往返。"""
        try:
            from neo4j import GraphDatabase  # type: ignore
        except ImportError as exc:
            raise RuntimeError("当前 Python 环境没有安装 neo4j 驱动，无法自动导入。") from exc

        node_groups: dict[str, list[dict[str, Any]]] = {}
        for node in nodes:
            node_type = required_str(node, "type")
            node_groups.setdefault(node_type, []).append(
                {"id": required_str(node, "id"), "props": normalize_node_properties(node)}
            )
        ...
        for edge in edges:
            ...
            if not relation_name:
                raise ValueError(f"关系 {required_str(edge, 'id')} 缺少关系类型。")
            edge_groups.setdefault(relation_name, []).append(
                {
                    "id": required_str(edge, "id"),
                    "source": required_str(edge, "source"),
                    "target": required_str(edge, "target"),
                    "props": normalize_edge_properties(edge, relation_name),
                }
            )
        ...
        try:
            with driver.session(database=self.database) if self.database else driver.session() as session:
                ...
                for node_type, rows in sorted(node_groups.items()):
                    label = safe_cypher_identifier(node_type, "节点标签")
                    query = (
                        f"UNWIND $rows AS row\n"
                        f"MERGE (n:KnowledgeNode:{label} {{id: row.id}})\n"
                        "SET n += row.props"
                    )
                    for batch in chunked(rows, batch_size):
                        session.run(query, rows=batch).consume()

                for relation_name, rows in sorted(edge_groups.items()):
                    rel_type = safe_cypher_identifier(relation_name, "关系类型")
                    query = (
                        "UNWIND $rows AS row\n"
                        "MATCH (source:KnowledgeNode {id: row.source})\n"
                        "MATCH (target:KnowledgeNode {id: row.target})\n"
                        f"MERGE (source)-[r:{rel_type} {{id: row.id}}]->(target)\n"
                        "SET r += row.props"
                    )
                    for batch in chunked(rows, batch_size):
                        session.run(query, rows=batch).consume()
...
def normalize_node_properties(node: dict[str, Any]) -> dict[str, Any]:
    ...
    # 课程中心图谱依赖这些属性区分五棵局部知识树。旧版入库器只保留
    # 通用属性，会导致 Neo4j 中无法按课程筛选或稳定绘制课程子图。
    course_centered_keys = (
        "course_id",
        "local_id",
        "catalog_id",
        "catalog_managed",
        ...
        "structure_kind",
        "original_code_structure_id",
        "scope_example_id",
        "code_quality_score",
    )
    for key in course_centered_keys:
        value = node.get(key)
        if value is None:
            continue
        if isinstance(value, (str, bool, int, float)):
            ...
            properties[key] = [str(item) for item in value if item is not None]
    ...
    if args.execute:
        ...
        if not password:
            ...
            try:
                ...
                # 正式图谱规模较大，采用批量 UNWIND 写入；生成的 Cypher 文件仍保留逐条版，
                # 便于在 Neo4j Browser 中人工检查某一条节点或关系。
                executor.execute_graph(builder.nodes, builder.edges, builder.clear_before_import)
