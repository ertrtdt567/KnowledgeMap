...
ENTITY_TYPE_ALIASES = {
    ...
    "概念": "KnowledgePoint",
    "编程概念": "KnowledgePoint",
    "面向对象概念": "KnowledgePoint",
    "oopconcept": "KnowledgePoint",
...
# 代码标识符具有大小写和字符级语义，不能套用普通术语的模糊归一化规则。
CASE_SENSITIVE_ENTITY_TYPES = {"CodeStructure", "CodeExample"}

# equivalent_to 来自自动抽取时，只允许人工确认过的真同义词触发实体合并。
# 未列入这里的候选仍可作为关系进入后续审核，但不会破坏两个独立节点。
CURATED_EQUIVALENT_MERGE_PAIRS = {
    frozenset({"Encoding(编码)", "字符编码"}),
    frozenset({"Jakarta EE", "Java EE"}),
    frozenset({"Latin-1", "ISO-8859-1"}),
    frozenset({"指针", "Pointers"}),
}
...
RELATION_ALIASES = {
    ...
    "EXPRESSES_CONCEPT": "expresses_concept",
    "具有示例代码": "has_code_example",
    "示例代码": "has_code_example",
    "has_code_example": "has_code_example",
    "HAS_CODE_EXAMPLE": "has_code_example",
    "用于示例": "used_in_example",
    "used_in_example": "used_in_example",
    "USED_IN_EXAMPLE": "used_in_example",
    "出现于示例": "appears_in_example",
    "appears_in_example": "appears_in_example",
    "APPEARS_IN_EXAMPLE": "appears_in_example",
    "考察": "assesses",
    "测试": "assesses",
    "assesses": "assesses",
    ...
    "IMPLEMENTS_INTERFACE": "implements_interface",
    "核心概念": "has_core_concept",
    "核心特点": "has_core_feature",
    "控制结构": "has_control_structure",
    "实现机制": "has_implementation_mechanism",
    "典型数据结构": "has_data_structure",
    "异常处理方式": "has_error_handling",
    "持久化方式": "has_persistence_mechanism",
    "工程实践方式": "has_engineering_practice",
    "并发机制": "has_concurrency_mechanism",
}


# 关系内部码用于抽取、校验、融合与去重；中文名称仅用于前端和 Neo4j 展示。
# 所以调整展示语言不会改变已有的质量控制逻辑。
RELATION_DISPLAY_NAMES = {
    ...
    "has_core_feature": "核心特点",
    "has_control_structure": "控制结构",
    "has_implementation_mechanism": "实现机制",
    "has_data_structure": "典型数据结构",
    "has_error_handling": "异常处理方式",
    "has_persistence_mechanism": "持久化方式",
    "has_engineering_practice": "工程实践方式",
    "has_concurrency_mechanism": "并发机制",
}
...
class GraphNormalizer:
    ...
    def normalize(self, raw_entities: list[dict[str, Any]], raw_relations: list[dict[str, Any]]) -> tuple[dict[str, Any], dict[str, Any]]:
        ...
        self._progress("规则融合", f"待处理实体 {len(prepared_entities)}")
    ...
    def _merge_by_rules(self, entities: list[PreparedEntity], union_find: UnionFind) -> None:
        ...
        for item in entities:
            groups[entity_key(item.type, item.name)].append(item)
    ...
    def _merge_by_equivalent_relations(self, entities: list[PreparedEntity], raw_relations: list[dict[str, Any]], union_find: UnionFind) -> None:
        ...
        for relation in raw_relations:
            ...
            if head.raw_id == tail.raw_id:
                continue
            pair = frozenset({head.name, tail.name})
            if pair not in CURATED_EQUIVALENT_MERGE_PAIRS:
                self.report["rejected_entity_merges"].append(
                    {
                        "method": "unreviewed_equivalent_relation",
                        "members": [head.original_name, tail.original_name],
                        "reason": "自动抽取的 equivalent_to 未经人工白名单确认，不执行实体合并。",
                    }
                )
                continue
    ...
    def _merge_by_llm(
        ...
        candidate_groups = [
            group
            for group in build_ambiguous_entity_groups(entities)
            if not same_union_group(group, union_find)
        ]
        total = min(len(candidate_groups), self.max_llm_entity_groups)
        if total:
            self._progress("实体消歧", f"共 {total} 组需要模型复核")
        for checked, candidate_group in enumerate(candidate_groups[:total], start=1):
            names = " / ".join(item.original_name for item in candidate_group[:3])
            self._progress("实体消歧", f"第 {checked}/{total} 组：{names[:120]}")
            try:
            ...
            if not self._accept_entity_merge_decision(decision):
                continue
            if not is_safe_semantic_merge_group(candidate_group):
                self.report["rejected_entity_merges"].append(
                    {
                        "method": "compound_concept_guard",
                        "members": [item.original_name for item in candidate_group],
                        "reason": "候选名称含并列复合概念，不能自动合并到其中一个子概念。",
                    }
                )
                continue
    ...
    def _build_edges(
        ...
        self._progress("关系筛选", f"共 {total_relations} 条关系")

        for relation_index, relation in enumerate(raw_relations, start=1):
            if relation_index == 1 or relation_index % 50 == 0 or relation_index == total_relations:
                self._progress(
                    "关系筛选",
                    f"第 {relation_index}/{total_relations} 条，已发起模型复核 {llm_relation_reviews}/{self.max_llm_relations}",
                )
            confidence = safe_float(relation.get("confidence", 0.0))
            ...
            if needs_llm and self.llm.enabled and llm_relation_reviews < self.max_llm_relations:
                ...
                self._progress(
                    "关系复核",
                    f"第 {llm_relation_reviews}/{self.max_llm_relations} 条：{source_node.name} -> {target_node.name}",
                )
            ...
            edge = StandardEdge(
                ...
                target=target_id,
                type=relation_type,
                relation_name=RELATION_DISPLAY_NAMES[relation_type],
                neo4j_type=RELATION_DISPLAY_NAMES[relation_type],
                confidence=round(confidence, 4),
    ...
    def _build_standard_graph(nodes: list[StandardNode], edges: list[StandardEdge]) -> dict[str, Any]:
        ...
        return {
            ...
            "schema": {
                ...
                "edge_display_names": {
                    relation: RELATION_DISPLAY_NAMES[relation] for relation in sorted(RELATION_TYPES)
                },
...
def entity_key(entity_type: str, name: str) -> tuple[str, str]:
    ...
    if normalized_type in CASE_SENSITIVE_ENTITY_TYPES:
        return (normalized_type, normalize_entity_name(name))
    return (normalized_type, comparable_name(name))
...
def build_ambiguous_entity_groups(entities: list[PreparedEntity]) -> list[list[PreparedEntity]]:
    ...
    for item in entities:
        if item.type in CASE_SENSITIVE_ENTITY_TYPES or item.type == "SyntaxRule":
            continue
...
def is_safe_semantic_merge_group(group: list[PreparedEntity]) -> bool:
    """阻止把“封装与访问控制”一类复合主题合并为单个子概念。"""
    names = [normalize_entity_name(item.name) for item in group]
    for longer in names:
        for shorter in names:
            if longer == shorter or shorter.lower() not in longer.lower():
                continue
            remainder = re.sub(re.escape(shorter), "", longer, flags=re.IGNORECASE)
            if re.search(r"[与和及、&/]", remainder):
                return False
    return True
