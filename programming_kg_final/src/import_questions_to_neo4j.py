...
class QuestionCypherBuilder:
    ...
    def build_import_statements(self) -> list[str]:
        statements = [
            ...
            "CREATE CONSTRAINT question_ability_name IF NOT EXISTS\nFOR (a:QuestionAbility)\nREQUIRE a.name IS UNIQUE",
        ...
        if self.clear_question_layer:
            statements.extend(
                [
                    ...
                    "MATCH (a:QuestionAbility) DETACH DELETE a",
                    # 兼容清理历史导入器产生、且已不再连接任何节点的答题能力节点。
                    "MATCH (a:Ability) WHERE NOT (a)--() DELETE a",
    ...
    def build_demo_queries_text(self) -> str:
        demo_question_id = (
            required_str(self.questions[0], "question_id") if self.questions else ""
        )
        queries = [
            (
                "1. 查看习题-知识点整体网络",
            ...
            (
                "3. 查看一道题考察什么",
                f"MATCH p=(q:Question {{id: {cypher_literal(demo_question_id)}}})-[r]->(n)\nRETURN p\nLIMIT 50;",
            ),
            (
                "4. 查询代码阅读题及其知识点",
    ...
    def _ability_statement(self, question_id: str, ability_name: str) -> str:
        return (
            f"MATCH (q:Question {{id: {cypher_literal(question_id)}}})\n"
            f"MERGE (a:QuestionAbility {{name: {cypher_literal(ability_name)}}})\n"
            "SET a:Ability\n"
    ...
    def _knowledge_link_statement(self, question_id: str, link: dict[str, Any], mapping_method: str) -> str:
        ...
        # 新版映射可选提供权重、认知层级和审核状态；历史链接不含这些字段仍完全兼容。
        if "role_weight" in link:
            props["role_weight"] = safe_float(link.get("role_weight"))
        if str(link.get("cognitive_level", "")).strip():
            props["cognitive_level"] = str(link["cognitive_level"])
        if str(link.get("mapping_status", "")).strip():
            props["mapping_status"] = str(link["mapping_status"])
        return (
            f"MATCH (q:Question {{id: {cypher_literal(question_id)}}})\n"
...
class Neo4jExecutor:
    ...
    def execute(self, statements: list[str]) -> None:
        try:
            ...
            self._execute_with_cypher_shell(statements)
            return
    ...
    def _execute_with_cypher_shell(self, statements: list[str]) -> None:
        ...
        if shell is None:
            raise RuntimeError(
                "当前 Python 环境没有 neo4j 驱动，且未找到 Neo4j Desktop cypher-shell。"
            )

        script_path: Path | None = None
        try:
            with tempfile.NamedTemporaryFile(
                "w", encoding="utf-8", delete=False, suffix=".cypher"
            ) as handle:
                for statement in statements:
            ...
            if self.database:
                command.extend(["-d", self.database])
            ...
            if result.returncode != 0:
                raise RuntimeError(result.stderr.strip() or "cypher-shell 自动导入失败。")
        finally:
