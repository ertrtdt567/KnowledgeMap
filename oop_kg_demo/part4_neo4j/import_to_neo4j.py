...
DEFAULT_INPUT_GRAPH = "work/oop_kg_demo/output/graph_normalized/standard_graph.json"
...
class Neo4jCypherBuilder:
    ...
    def build_demo_queries_text(self) -> str:
        ...
        queries = [
            ...
            (
                "3. 语法与概念对应",
                "MATCH p=(s:SyntaxRule)-[:EXPRESSES_CONCEPT|HAS_SYNTAX]-(c:OOPConcept)\n"
                "RETURN p\n"
                "LIMIT 50;",
            ),
            (
                "4. 学习路径查询",
                "MATCH p=(:OOPConcept {name: '继承'})-[:PREREQUISITE_OF*1..3]->(:OOPConcept)\n"
                "RETURN p\n"
                "LIMIT 50;",
            ),
            (
                "5. Java 与 C++ 差异",
                "MATCH p=(:ProgrammingLanguage {name: 'Java'})-[:DIFFERS_FROM]-(:ProgrammingLanguage)\n"
                "RETURN p\n"
                "LIMIT 20;",
            ),
            (
    ...
    def _edge_statement(self, edge: dict[str, Any]) -> str:
        ...
        rel_type = str(edge.get("neo4j_type") or edge.get("type") or "").upper()
        rel_type = safe_cypher_identifier(rel_type, "关系类型")
        props = normalize_edge_properties(edge, rel_type)
        return (
...
def build_report(
    ...
    return {
        "schema_version": "oop_kg_neo4j_import_v1",
        ...
        "relationship_type_strategy": "Uppercase schema relation type",
