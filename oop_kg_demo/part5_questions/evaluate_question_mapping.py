...
DEFAULT_OUTPUT = "work/oop_kg_demo/output/question_mapping/question_mapping_evaluation.json"
...
def build_report(
    ...
    return {
        "schema_version": "oop_kg_question_mapping_evaluation_v1",
...
def main(argv: list[str]) -> int:
    ...
    per_question = [evaluate_question(question, mapping_by_id.get(question["question_id"], {})) for question in questions]
    report = build_report(questions_path, links_path, output_path, per_question)
    write_json(output_path, report)

    metrics = report["metrics"]
    print(f"评估题目数：{report['question_count']}")
